    ...
    if (mysqli_num_rows($result) === 1) {
        ...
        if (isset($_POST['update'])) {
            ...
            if ($senha !== $confirmar_senha) {
                ...
                $updateQuery = mysqli_prepare($link, "UPDATE Laboratory SET senha_hash=? WHERE idLab=?");
                mysqli_stmt_bind_param($updateQuery, "si", $senhahash, $id);
                $updateResult = mysqli_stmt_execute($updateQuery);
        
                if ($updateResult) {
                    $updatetempKeyQuery = mysqli_prepare($link, "UPDATE Laboratory SET tempKey = NULL WHERE idLab = ?");
